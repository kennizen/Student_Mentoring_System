...
const AdminDashboard = () => {
    ...
    const [admin, setAdmin] = useState(JSON.parse(localStorage.getItem("authData")));
    ...
    if (!admin) {
        // redirecting back to home if token does'nt exist
        history.push("/");
    }

    // fetching the admin details
    useEffect(() => {
        dispatch(adminGetDetails());
    }, [dispatch]);
        ...
        <div className="h-screen flex bg-gray-50">
            ...
            <div className="w-17/20 h-screen">
                ...
                <div className="flex items-center justify-center">
                    ...
                    {route.home && <Home />}
                    {route.profile && <Profile details={user} />}
                </div>
            </div>
        </div>
    );
